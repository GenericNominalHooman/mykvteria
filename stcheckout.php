...
<?php
	...
	if(!empty($_POST)){
		...
		$sql = "INSERT INTO storders (username, status) VALUES (?, 'Pending')";
		...
		$sql = "INSERT INTO storderlist (order_id, menu_name, price, quantity, total) VALUES (?,?,?,?,?)";
		...
		for ($i = 0; $i < count($hidden_name); $i++) {
			...
			mysqli_query($con, "delete from stcart where username = '$username'");
		}
